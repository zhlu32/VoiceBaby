...
public abstract class AbsCallReceiver extends BroadcastReceiver {
	...
	protected void onIncomingCallOffhook(Context ctx, String number, Date hookDate) {
	}

	protected void onOutgoingCallOffhook(Context ctx, String number, Date hookDate) {
	}
	...
	public void onCallStateChanged(Context context, int state, String number) {
		...
		case TelephonyManager.CALL_STATE_OFFHOOK:
			...
			if (lastState != TelephonyManager.CALL_STATE_RINGING) {
				isIncoming = false;
				callStartTime = new Date();
				onOutgoingCallOffhook(context, savedNumber, callStartTime);
			}else {
				isIncoming = true;
				onIncomingCallOffhook(context, savedNumber, new Date());
			}
			break;
